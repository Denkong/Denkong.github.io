...
function Fish2(){
	...
	this.render = function(ctx,mouse){
		for(var i = 0;i < maxID;i++){
			...
			if(objects[i].name != 'four') {continue;}
			var obj = objects[i];
			
			
			if (obj.theway['x']==-1) {obj.x-=obj.speed;ctx.drawImage(FourFishPng,obj.x-FirstFishPng1.width/2,obj.y-FirstFishPng1.height/2)}
			if (obj.theway['x']==1) {obj.x+=obj.speed;ctx.drawImage(FourFishPng1,obj.x-FirstFishPng.width/2,obj.y-FirstFishPng.height/2)}
